...
%>
<h1><%=role == 1 ? "Staff" : "Customer" %>  management</h1>
	<hr>
	<div class="row">
		<div class="col-md-12">
				<!-- Button trigger modal -->
			<button type="button" class="btn btn-primary btn-add table-action">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> Add new <%=role == 1 ? "Staff" : "Customer" %>
			</button>
...
<div class="modal fade" id="dataModal" tabindex="-1" role="dialog"
	...
	<script>
		var TableData = {
			saveUrl: App.contextPath + "/rest/user/save",
			deleteUrl: App.contextPath + "/rest/user/delete",
			findUrl: App.contextPath + "/rest/user/find",
			listUrl : App.contextPath + "/rest/user/findbyrole?role=<%=role%>",
			showAddForm: function() {
				$("#dataModelTitle").html("Add <%=role == 1 ? "Staff" : "Customer" %>");
